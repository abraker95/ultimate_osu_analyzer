...
class ManiaHitOffsets():
    ...
    def __set_data(self, beatmap_path, replay_path):
        ...
        score_data = ManiaScoreData.get_score_data(replay_data, map_data)

        self.win.setWindowTitle(beatmap.metadata.name + ' ' + replay.get_name())

        hitp_scores    = score_data.loc[0][score_data.loc[0]['type'] != ManiaScoreData.TYPE_MISS]
        timing_data    = hitp_scores['replay_t'].values
        hitoffset_data = (hitp_scores['replay_t'] - hitp_scores['map_t']).values
