...
class ScatterGraph(pyqtgraph.PlotWidget):
    ...
    def __init__(self):
        ...
        self.setLimits(xMin=ScatterGraph.MIN_TIME)
        self.setRange(xRange=(-100, 10000))

        self.getViewBox().setMouseEnabled(x=False, y=False)

        self.plot_data_item = pyqtgraph.ScatterPlotItem(size=10, pen=pyqtgraph.mkPen(None), brush=pyqtgraph.mkBrush(255, 255, 255, 120))
        self.addItem(self.plot_data_item)

        self.timeline_marker = pyqtgraph.InfiniteLine(angle=90, movable=True)
        self.timeline_marker.setBounds((ScatterGraph.MIN_TIME + 100, None))
    ...
    def update_data(self, data=None):
        ...
        self.plot_data_item.addPoints(x=data_x, y=data_y)
